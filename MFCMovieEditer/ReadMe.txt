================================================================================
    MFC (MICROSOFT FOUNDATION CLASS) ライブラリ : MFCMovieEditer プロジェクトの概要
===============================================================================

この MFCMovieEditer アプリケーションは、アプリケーション ウィザードによって作成されました。このアプリケーションは MFC の基本的な使い方を示すだけでなく、アプリケーションを作成するための開始点でもあります。

このファイルには、MFCMovieEditer アプリケーションを構成する各ファイルの内容の概要が含まれています。

MFCMovieEditer.vcxproj
...
MFCMovieEditer.vcxproj.filters
...
MFCMovieEditer.h
    ...
    その他のプロジェクト固有のヘッダー (Resource.h など) が含まれ、CMFCMovieEditerApp アプリケーション クラスを宣言します。

MFCMovieEditer.cpp
    これはメインのアプリケーション ソース ファイルで、CMFCMovieEditerApp アプリケーション クラスが含まれています。
...
res\MFCMovieEditer.ico
    これはアプリケーションのアイコンとして使用されるアイコン ファイルです。このアイコンは、メイン リソース ファイル MFCMovieEditer.rc にインクルードされます。

res\MFCMovieEditer.rc2
...
MFCMovieEditerDlg.h, MFCMovieEditerDlg.cpp - ダイアログ
    これらのファイルには、CMFCMovieEditerDlg クラスが含まれます。このクラスはアプリケーションのメイン ダイアログの動作を定義します。ダイアログのテンプレートは、Microsoft Visual C++ で編集できる MFCMovieEditer.rc に含まれています。
...
StdAfx.h, StdAfx.cpp
    これらのファイルは、MFCMovieEditer.pch という名前のプリコンパイル済みヘッダー (PCH) ファイルと、StdAfx.obj という名前のプリコンパイル済みの型ファイルをビルドするために使用されます。

Resource.h
    これは、新しいリソース ID を定義する標準のヘッダー ファイルです。このファイルの読み込みおよび更新は、Microsoft Visual C++ で行います。

MFCMovieEditer.manifest
